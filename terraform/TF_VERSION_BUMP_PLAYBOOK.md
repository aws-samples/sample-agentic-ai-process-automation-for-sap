...
`terraform/VERSION` is a compatibility declaration — it states which FAST version the Terraform code is verified compatible with. It always includes a `-tf.N` suffix that matches the corresponding git tag.
...
1. Update `terraform/VERSION` with the new version
2. Update `CHANGELOG.md` with `[Terraform]` labeled entries
3. Commit, tag with `v<VERSION>`, and push

```bash
git tag v$(cat terraform/VERSION)
git push origin v$(cat terraform/VERSION)
```
