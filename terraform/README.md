...
# Terraform Infrastructure for the Agentic ERP Automation Quickstart

This directory contains Terraform configurations for deploying the Agentic ERP Automation Quickstart.

> **Deployment guide:** For step-by-step deployment instructions, see [Terraform Deployment Guide](../docs/getting-started/TERRAFORM_DEPLOYMENT.md). This README covers module architecture, configuration reference, and developer documentation.
...
See the [Terraform Deployment Guide](../docs/getting-started/TERRAFORM_DEPLOYMENT.md) for detailed instructions, VPC deployment, troubleshooting, and cleanup.

> **Auth profiles:** The inbound JWT authorizer is driven by `auth_profile`. The default
> `cognito-basic` needs no extra config. For an `entra`/`okta` inbound profile, set
> `auth_inbound_discovery_url` + `auth_inbound_allowed_clients` in `terraform.tfvars` — the
> `external` data source in `emit.tf` resolves them at plan time (requires `python` with
> PyYAML on PATH). See [`auth-profiles.yaml`](../auth-profiles.yaml) for the profile catalog.
>
> **Terraform scope:** Terraform consumes only the **inbound axis**. The outbound (SAP MCP
> target), mode (batch runner), and frontend (direct-IdP SPA) axes are CDK-only — no Terraform
> module exists for those. Selecting a profile that needs those axes fails `terraform plan`
> with a clear message. See [Auth Profile Selection](../docs/sap/AUTH_PROFILE_SELECTION.md#terraform-scope).
...
| `backend_pattern` | Agent pattern to deploy | `"agent"` |
...
| `auth_profile` | Inbound auth profile from auth-profiles.yaml | `"cognito-basic"` |
| `auth_inbound_discovery_url` | OIDC discovery URL (entra/okta inbound only) | `""` |
| `auth_inbound_allowed_clients` | Allowed client IDs (entra/okta inbound only) | `[]` |
...
> **Note:** Feedback and OAuth2 provider Lambda code is shared from `cdk/lambdas/`. The zip-packager Lambda is Terraform-specific and lives under `terraform/lambdas/`.
