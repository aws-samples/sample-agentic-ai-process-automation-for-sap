...
# nosemgrep: aws-dynamodb-table-unencrypted — uses AWS-managed encryption; CMK is a production hardening step
...
resource "aws_lambda_function" "feedback" {
  ...
  # nosemgrep: aws-lambda-environment-unencrypted — quick-start uses AWS-managed keys
...
# -----------------------------------------------------------------------------
# The deployment resource (aws_api_gateway_deployment.full_api) lives in
# api_routes.tf — it's shared across all REST API endpoints, not just feedback.

# nosemgrep: missing-api-gateway-cache-cluster — caching not needed for feedback API

resource "aws_api_gateway_stage" "prod" {
  stage_name    = "prod"
  rest_api_id   = aws_api_gateway_rest_api.feedback.id
  deployment_id = aws_api_gateway_deployment.full_api.id
