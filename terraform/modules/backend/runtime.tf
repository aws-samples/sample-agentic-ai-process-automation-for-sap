...
# Always created (no count) so the runtime's replace_triggered_by can reference it in
# both modes. In zip mode the value is static ("zip"), so it never triggers a replacement.
resource "terraform_data" "docker_image_hash" {
  input = local.is_docker && var.container_uri == null ? sha256(join("", concat(
    ...
    [for f in fileset("${local.project_root}/agentcore/agent/utils", "**/*.py") : filesha256("${local.project_root}/agentcore/agent/utils/${f}")],
    [for f in fileset("${local.project_root}/agentcore/gateway", "**/*.py") : filesha256("${local.project_root}/agentcore/gateway/${f}")],
...
resource "null_resource" "docker_build_push" {
  ...
  provisioner "local-exec" {
    ...
    command     = <<-EOT
      ...
      DOCKERFILE="agentcore/${var.backend_pattern}/Dockerfile"
...
data "aws_iam_policy_document" "runtime_policy" {
  ...
  # Reads OAuth2 credentials from the Token Vault secret created by AgentCore
  # Identity — not the machine client secret directly.
  statement {
  ...
  # The @requires_access_token decorator first looks up provider metadata
  # (GetOauth2CredentialProvider), then fetches the access token from the Token
  # Vault (GetResourceOauth2Token) — both actions are required.
  statement {
...
resource "aws_bedrockagentcore_agent_runtime" "main" {
  ...
  # In VPC mode, the target VPC must have endpoints for the AWS services this runtime
  # calls — see docs/getting-started/DEPLOYMENT.md for the required list.
  ...
  authorizer_configuration {
    custom_jwt_authorizer {
      ...
      allowed_clients = local.resolved_inbound_runtime_clients
    }
  }
  ...
  # Authorization header must be allowlisted for the JWT sub claim to reach the runtime.
  ...
  environment_variables = merge(
    {
      ...
      # Demo gate: skill router skips example_* skills unless demo is enabled.
      DEMO_ENABLED = var.demo_enabled ? "true" : "false"
    },
    # Contact directory for SOP placeholder substitution
