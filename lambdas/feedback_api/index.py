...
class FeedbackRequest(BaseModel):
    ...
    def validate_session_id_format(cls, v: str) -> str:
        """Restrict session_id to alphanumeric characters, hyphens, and underscores."""
...
def submit_feedback() -> Dict[str, Any]:
    try:
        ...
        # Extract user ID from the authorizer context. Two shapes:
        #   Cognito authorizer  -> claims nested under authorizer["claims"]
        #   JWT Lambda authorizer (okta/entra) -> fields flat on the context
        request_context = app.current_event.request_context
        authorizer = request_context.authorizer or {}
        ...
        item = {
            ...
            "message": {"S": feedback_data.message},
            "userId": {"S": user_id},
        ...
        if feedback_data.comment:
            item["comment"] = {"S": feedback_data.comment}
