...
"""
Ephemeral CodeBuild deployment script.

Deploys the full stack using a temporary CodeBuild project.
...
def create_codebuild_iam_role(role_name: str, boundary_arn: str) -> str:
    ...
    time.sleep(10)  # nosemgrep: arbitrary-sleep — IAM eventual consistency
    return role_arn
...
def create_codebuild_project(
    ...
    region: str,
    auth_profile: str,
    auth_inbound_discovery_url: str,
    auth_inbound_allowed_clients: str,
) -> None:
    """
    ...
    buildspec: str = (
        ...
        "      - cd $CODEBUILD_SRC_DIR/cdk && npm ci\n"
        "  build:\n"
        "    commands:\n"
        '      - echo "Source dir contents:" && ls -la $CODEBUILD_SRC_DIR/\n'
        "      - cd $CODEBUILD_SRC_DIR && python scripts/deploy/run_emit.py --backend cdk\n"
        "      - cd $CODEBUILD_SRC_DIR/cdk && cdk bootstrap\n"
        "      - cd $CODEBUILD_SRC_DIR/cdk && cdk deploy --all --require-approval never\n"
        "  post_build:\n"
        "    commands:\n"
        "      - cd $CODEBUILD_SRC_DIR && python scripts/deploy/deploy-frontend.py\n"
    )
    ...
    project_input: Dict[str, Any] = {
        ...
        "environment": {
            ...
            "environmentVariables": [
                ...
                {"name": "AUTH_PROFILE", "value": auth_profile, "type": "PLAINTEXT"},
                {
                    "name": "AUTH_INBOUND_DISCOVERY_URL",
                    "value": auth_inbound_discovery_url,
                    "type": "PLAINTEXT",
                },
                {
                    "name": "AUTH_INBOUND_ALLOWED_CLIENTS",
                    "value": auth_inbound_allowed_clients,
                    "type": "PLAINTEXT",
                },
            ],
        },
...
def stream_build_logs(build_id: str) -> str:
    ...
    while True:
        ...
        time.sleep(
            LOG_POLL_INTERVAL
        )  # nosemgrep: arbitrary-sleep — polling build status
...
def main() -> int:
    ...
    # 'project' is retained after the run (not cleaned up), so it lives in a
    # plain local; only role/boundary/bucket need atexit tracking.
    ...
    config_path = Path(__file__).parent.parent.parent / "cdk" / "config.yaml"
    ...
    if not stack_name:
        log_error("'stack_name_base' not found in cdk/config.yaml")
        return 1
    log_success(f"Stack name: {stack_name}")

    # Read the selected auth profile + inbound overrides (regex — this script
    # stays PyYAML-free). These are forwarded to CodeBuild as env vars so
    # run_emit.py resolves the profile in the build (cdk/config.yaml is
    # git-ignored and not in the deployed zip). Env vars set by the operator
    # take precedence over these config-derived defaults.
    import re
    ...
    # inbound_overrides (entra/okta only) — discovery_url scalar + allowed_clients
    # inline list. Empty for the cognito default (backend fallback path).
    auth_inbound_discovery_url = os.environ.get("AUTH_INBOUND_DISCOVERY_URL", "")
    if not auth_inbound_discovery_url:
        d = re.search(r"^\s+discovery_url:\s*(\S+)", config_text, re.MULTILINE)
        auth_inbound_discovery_url = d.group(1) if d else ""

    auth_inbound_allowed_clients = os.environ.get("AUTH_INBOUND_ALLOWED_CLIENTS", "")
    if not auth_inbound_allowed_clients:
        a = re.search(r"^\s+allowed_clients:\s*\[([^\]]*)\]", config_text, re.MULTILINE)
        if a:
            auth_inbound_allowed_clients = ",".join(
                c.strip().strip("\"'") for c in a.group(1).split(",") if c.strip()
            )

    # Generate unique resource names
    ts: str = str(int(time.time()))
    project_name: str = f"{RESOURCE_PREFIX}-{ts}"
    ...
    create_codebuild_project(
        ...
        auth_inbound_discovery_url=auth_inbound_discovery_url,
        auth_inbound_allowed_clients=auth_inbound_allowed_clients,
    )
    ...
    if final_status == "SUCCEEDED":
        ...
        # AmplifyUrl lives on the split frontend stack; fall back to the
        # legacy single-stack name for older deployments.
        for candidate in (f"{stack_name}-frontend", stack_name):
            try:
                outputs = get_stack_outputs(stack_name=candidate)
    ...
    log_info(
        f"CodeBuild project '{project_name}' retained for debugging. "
        f"View in console: https://{region}.console.aws.amazon.com/codesuite/codebuild/projects/{project_name}"
    )
    log_info(
        f"To delete it manually: aws codebuild delete-project --name {project_name}"
    )
