...
from _deploy_common import (  # noqa: E402
    get_stack_outputs,
    log_error,
    log_info,
    log_success,
...
from run_emit import (  # noqa: E402  (sibling module in scripts/deploy)
    _cdk_inputs,
    run_emit,
)
...
def _resolve_oidc_config(
    outputs: Dict[str, str], region: str, frontend_block: Optional[dict]
) -> Dict[str, str]:
    """Resolve the SPA's OIDC config (metadata_url or authority / client_id / scope).

    frontend_block present (direct-entra/direct-okta): external IdP — the block's
    discovery_url is passed through verbatim as metadata_url (no 'authority' key),
    client_id from the block, scope from the block or 'email openid profile'.
    No Cognito outputs required.

    frontend_block absent (cognito / cognito+federated): authority is built from
    the Cognito pool URL; requires CognitoUserPoolId + CognitoClientId."""
    if frontend_block:
        # External IdP: hand oidc-client-ts the discovery URL verbatim as
        # metadata_url. It fetches metadata from that exact URL (no authority
        # suffix round-trip), so Okta custom auth-server paths and trailing
        # slashes work.
        return {
            "metadata_url": frontend_block["discovery_url"],
            # Direct issuers carry authority (the issuer) too — oidc-client-ts needs
            # it at signin even with metadata_url. Conditional so a block without
            # authority still resolves authority-free.
            **(
                {"authority": frontend_block["authority"]}
                if frontend_block.get("authority")
                else {}
            ),
            "client_id": frontend_block["client_id"],
            "scope": frontend_block.get("scope", "email openid profile"),
        }
    missing = [k for k in ("CognitoUserPoolId", "CognitoClientId") if k not in outputs]
    if missing:
...
def generate_aws_exports(
    ...
    pattern: str,
    frontend_dir: Path,
    frontend_block: Optional[dict] = None,
) -> None:
    """
    ...
    """
    required = ["AmplifyUrl", "RuntimeArn", "FeedbackApiUrl"]
    missing = [k for k in required if k not in outputs]
    if missing:
        raise ValueError(f"Missing required stack outputs: {', '.join(missing)}")

    oidc = _resolve_oidc_config(outputs, region, frontend_block)

    aws_exports = {
        # Cognito path yields "authority"; external-IdP path yields "metadata_url".
        **({"authority": oidc["authority"]} if "authority" in oidc else {}),
        **({"metadata_url": oidc["metadata_url"]} if "metadata_url" in oidc else {}),
        ...
        "awsRegion": region,
        "apiUrl": outputs["FeedbackApiUrl"].rstrip("/"),
        "feedbackApiUrl": outputs["FeedbackApiUrl"],
        "agentPattern": pattern,
    }

    # Ticketing and test-data are independent demo features. Ticketing (the
    # backend-hosted /tickets UI) is present when the backend emits TicketsTableName;
    # test-data (the separate demo stack) is present when it emits DemoApiUrl.
    aws_exports["ticketingEnabled"] = "TicketsTableName" in outputs
    aws_exports["testDataEnabled"] = "DemoApiUrl" in outputs
    if "DemoApiUrl" in outputs:
        aws_exports["demoApiUrl"] = outputs["DemoApiUrl"].rstrip("/")
...
def main() -> int:
    ...
    config_path = project_root / "cdk" / "config.yaml"
    ...
    # Fetch CDK outputs from all independent stacks
    log_info(f"Fetching configuration from CDK stacks: {stack_name}-*")
    try:
        outputs: Dict[str, str] = {}
        for suffix in ["frontend", "cognito", "backend", "demo"]:
            sub_stack = f"{stack_name}-{suffix}"
            try:
                sub_outputs = get_stack_outputs(sub_stack)
                outputs.update(sub_outputs)
                log_success(f"  Loaded outputs from {sub_stack}")
            except (subprocess.CalledProcessError, ValueError):
                log_info(f"  Stack {sub_stack} not found, trying parent stack")

        # Fallback: try the original single-stack name (older, non-split deployments)
        if not outputs:
            outputs = get_stack_outputs(stack_name)
            log_success(f"  Loaded outputs from {stack_name} (legacy single-stack)")

        region = (
            get_stack_region(f"{stack_name}-frontend")
            if "AmplifyAppId" in outputs
            else get_stack_region(stack_name)
        )
    ...
    # Emit the resolved-profile artifact from cdk/config.yaml so a direct-entra/
    # direct-okta frontend drives the SPA login issuer. Cognito frontends write no
    # frontend block -> generate_aws_exports falls back to the Cognito authority.
    artifact_path = project_root / ".auth-profile-resolved.json"
    profile_name, inbound_overrides, frontend_overrides, _ = _cdk_inputs()
    run_emit(
        profile_name,
        overrides=inbound_overrides,
        out_path=str(artifact_path),
        frontend_overrides=frontend_overrides,
        include_frontend=True,  # REQUIRED on the frontend-writing path: a direct-*
        # profile with missing overrides must fail loud here,
        # not deploy silently against a broken Cognito fallback.
    )
    frontend_block = None
    if artifact_path.exists():
        frontend_block = json.loads(artifact_path.read_text()).get("frontend")
    ...
    build_dir = frontend_dir / BUILD_DIR_NAME
    if not build_dir.exists():
        log_error(f"Build directory '{BUILD_DIR_NAME}' not found")
        return 1
    ...
    while True:
        ...
        time.sleep(10)  # nosemgrep: arbitrary-sleep — polling Amplify deployment status
