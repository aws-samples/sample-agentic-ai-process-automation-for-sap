...
def start_local_agent(
    ...
    Args:
        ...
        pattern (str): Agent pattern name (the sample ships 'agent')
    ...
    agent_path = Path(__file__).parent.parent / pattern / agent_file

    if not agent_path.exists():
        print_msg(f"Agent file not found: {agent_path}", "error")
        sys.exit(1)

    # --pattern is user-controlled; reject if it resolves outside the project directory
    project_dir = Path(__file__).parent.parent
    try:
        agent_path.resolve().relative_to(project_dir.resolve())
    except ValueError:
        print_msg(
            f"Security error: Agent path outside project directory: {agent_path}",
            "error",
        )
        sys.exit(1)
    ...
    try:
        ...
        for i in range(30):  # up to 30 seconds
            ...
            time.sleep(1)  # nosemgrep: arbitrary-sleep — polling for port availability
...
Notes:
  ...
  - Local mode: Pattern read from cdk/config.yaml to start correct agent
  ...
  - Always runs in interactive conversation mode
    ...
    parser.add_argument(
        "--pattern",
        type=str,
        help="Override agent pattern from config (the sample ships 'agent')",
    )
...
def main():
    ...
    if args.local:
        pattern = args.pattern if args.pattern else stack_cfg.get("pattern", "agent")
        ...
        # AgentCore runtime uses a JWT authorizer, so send the access token, not the ID token
