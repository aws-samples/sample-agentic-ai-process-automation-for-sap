...
def make_api_request(
    ...
    try:
        if method == "POST":
            # nosemgrep: use-raise-for-status — test script inspects status codes directly
...
def main():
    ...
    # API Gateway's Cognito authorizer requires the ID token, not the access token
    ...
    # NOTE: Table name follows the pattern '{stack_name}-feedback' defined in cdk/lib/backend-stack.ts
