...
> **Note:** SAP OData access (including writes) is now handled by the external AWS for SAP MCP server, not Lambda-based tools or an SQS write path. This doc covers the machine-to-machine auth between AgentCore Runtime and Gateway, which remains current. See [ADR-012](../design-decisions/012-sap-mcp-server-integration.md).
