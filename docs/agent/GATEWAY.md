...
This document describes how this project implements AgentCore Gateway with Lambda targets to provide a scalable, production-ready tool execution architecture.
...
Each Lambda target follows this pattern:
...
Gateway delivers request headers via the Lambda context, not the event:

```python
headers = context.client_context.custom.get("bedrockAgentCorePropagatedHeaders", {})
# Returns: {"x-user-token": "Bearer ...", "x-audit-correlation-id": "...", ...}
```

Only headers listed in the Gateway's `allowedRequestHeaders` are propagated.
...
def lambda_handler(event, context):
    try:
        ...
        if tool_name == "sample_tool":
            name = event.get('name', 'World')
            result = f"Hello, {name}! This is a sample tool."
...
The gateway is created using native CloudFormation L1 constructs in `cdk/lib/backend-stack.ts`:
...
Agents connect to the Gateway using the Model Context Protocol (MCP). This project provides two integration approaches:
...
**Example:** See `agentcore/agent/basic_agent.py` for complete implementation.
...
- [Deployment Guide](../getting-started/DEPLOYMENT.md) - How to deploy the infrastructure
