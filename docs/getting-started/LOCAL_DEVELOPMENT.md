...
This guide explains how to run the full stack locally using Docker Compose for development purposes.

> **Note:** Docker is only needed for local development with Docker Compose. It is **not** required for deploying to AWS — see the [Deployment Guide](DEPLOYMENT.md).

## Prerequisites

**Important**: Local development still requires a deployed stack in AWS for backend dependencies (Memory, Gateway, SSM parameters). Docker Compose only containerizes the frontend and agent - it doesn't replace AWS services.

### Required

1. **Deployed Stack**: You must have already deployed to AWS using:
   ```bash
   cd cdk
   cdk deploy
   ```
...
### Swapping in Your Own Agent Framework

The sample ships a single Strands agent under `agent/`. To run a different framework
(LangGraph, Claude Agent SDK, etc.), add your implementation with its own `Dockerfile`
and point the compose service at it:

1. **Edit docker/docker-compose.yml**:
   ```yaml
   agent:
     build:
       dockerfile: <your-agent-dir>/Dockerfile
   ```
...
## Inspecting a Deployed Runtime

CDK owns this project's AgentCore lifecycle, so the `agentcore` CLI (bundled
transitively via `bedrock-agentcore-starter-toolkit`) can't deploy or invoke
here — there's no `.bedrock_agentcore.yaml` and you should **not** hand-author
one (it'd be a staleness-prone second source of truth next to SSM; invoke via
the chat UI or the boto3 eval harness instead). But a few of its **read-only**
verbs work against the already-deployed resources and fill real debugging gaps.

Grab the IDs the commands need from the deployed stack:

```bash
# Runtime (agent) ARN — the last segment is the agent id
aws ssm get-parameter --name "/${STACK_NAME}/runtime-arn" --query Parameter.Value --output text

# Memory id — from stack outputs (or the MEMORY_ID you already exported above)
```

- **Inspect stored conversation events** — no config file needed:
  ```bash
  agentcore memory show events -m "$MEMORY_ID" -r "$AWS_DEFAULT_REGION"
  agentcore memory browse       -m "$MEMORY_ID" -r "$AWS_DEFAULT_REGION"
  ```
  Note: `agentcore memory show records` is empty by design — memory is
  short-term only (`MemoryStrategies: []` in `cdk/lib/backend-stack.ts`). Use
  `events` / `browse`.

- **Triage runtime traces / errors** — not config-driven here, so pass the IDs
  explicitly (omitting them is a footgun):
  ```bash
  agentcore obs list --errors --agent-id <id-from-runtime-arn> --session-id <session-id>
  ```

- **Verify what CDK deployed** for the Gateway:
  ```bash
  agentcore gateway list-mcp-gateways
  agentcore gateway list-mcp-gateway-targets --gateway-id <id>
  ```
  Caveat: `agentcore identity get-cognito-inbound-token` only works on the
  Cognito inbound profile. Under the `entra-obo` profile the Gateway validates
  an Entra `aud`, so a Cognito token 401s.
...
- Add new tools in `agentcore/agent/tools/` or `agentcore/gateway/tools/`
- Update infrastructure in `cdk/`
